...
Regular analysis and Agent paths now also build a lightweight `event_context` inside the current project. `src/services/event_context.py` fetches and standardizes stock announcements, earnings previews/flash reports, stock news, research report titles/rating changes, and industry/board event summaries derived from existing fundamentals/board context. The LLM does not perform unconstrained web search for these events; it only consumes the injected structured items and `event_digest`. Events must include a parseable publish time and satisfy `publish_time <= as_of_date`; future-dated, unknown-time, or out-of-window non-announcement events are filtered. Announcement and earnings disclosures are treated as higher-priority confirmed evidence, while ordinary news, research titles, and derived industry events remain uncertain clues. Fetch failures only degrade `event_context.status` to `partial`/`failed` with warnings and do not block the main technical, factor, fundamentals, or LLM analysis flow.
